    ...
    case 'getZtree':
        ...
        if ($groupid > 0) {
            ...
            foreach($hosts as &$each_host) {
                ...
                $each_host['target'] = '_self';
                $each_host['click'] = 'javascript:getGraphs(\'host\',' . $each_host['hostid'] . ')';
            }
            unset($each_host);
            ...
            if ($hostid == 0) {
                ...
                foreach($groups as &$each) {
                    ...
                    $each['isParent'] = true;
                    $each['target'] = '_self';
                    $each['click'] = 'getGraphs(\'group\',' . $each['groupid'] . ')';
                    //查询下面有多少机器
                ...
                if (is_array($applications)) {
                    foreach($applications as &$each) {
                        $each['nocheck'] = true;
                        $each['target'] = '_self';
                        $each['click'] = 'getGraphs(\'application\',' . $each['applicationid'] . ')';
                    }
                }
